 ...
 *     files: Map<fileId, { fileId, name, size, infoHash, magnet, userId }>
 *   }
 * }
 */
...
function ensureSwarm(swarmHash) {
  if (!swarms.has(swarmHash)) {
    swarms.set(swarmHash, { clients: new Set(), users: new Map(), files: new Map() });
  }
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (data) => {
    ...
    if (msg.type === "join" && msg.swarmHash && msg.userId) {
      ...
      // Send current file list to new joiner (so re-joins get files without extra round-trip)
      const files = Array.from(swarm.files.values());
      if (files.length > 0) {
        safeSend(ws, { type: "file:list", files });
      }
    ...
    // Special handling for file announces & revokes & requestFiles
    const swarm = swarms.get(swarmHash);
    if (!swarm) return;

    if (msg.type === "file:announce") {
      // attach owner and timestamp
      const owner = ws.meta.userId || null;
      const meta = {
        fileId: msg.fileId,
        name: msg.name,
        size: msg.size,
      ...
      swarm.files.set(meta.fileId, meta);
      // Broadcast the announce to other peers
      msg.relayTs = nowMs();
      broadcastToSwarm(swarmHash, msg, ws);
      return;
    }

    if (msg.type === "file:revoke") {
      if (msg.fileId && swarm.files.has(msg.fileId)) {
        swarm.files.delete(msg.fileId);
      }
      msg.relayTs = nowMs();
      broadcastToSwarm(swarmHash, msg, ws);
      return;
    ...
    // Fallback: relay allowed messages
    msg.relayTs = nowMs();
    broadcastToSwarm(swarmHash, msg, ws);
  });
  ...
  ws.on("close", () => {
    ...
    if (swarmHash && swarms.has(swarmHash)) {
      ...
      // Remove files announced by this user
      for (const [fid, fmeta] of s.files.entries()) {
        if (fmeta.userId === userId) {
          s.files.delete(fid);
          // inform others that file is revoked
          broadcastToSwarm(swarmHash, { type: "file:revoke", fileId: fid });
        }
      }
